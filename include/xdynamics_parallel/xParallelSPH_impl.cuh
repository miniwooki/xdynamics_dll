#ifndef XPARALLELSPH_IMPL_CUH
#define XPARALLELSPH_IMPL_CUH
...
//}
//
inline __device__ double6 make_sym_tensor(double s0, double s1, double s2, double s3, double s4, double s5)
{
	double6 d6 = { s0, s1, s2, s3, s4, s5 };
	return d6;
}


__device__
unsigned int calcGridHash(int3 cell)
{
	if (scte.dim == 3){
...
__device__ int3 LoopStart(double3 pos)
{
	...
	return calcGridPos(pos - scte.kernel_support_radius);
}
...
__global__ void cuBoundaryMoving_kernel(
	unsigned int sid,
	unsigned int pcount,
	double stime,
	double3* pos,
	...
	double3* auxVel)
{
	...
	if (id >= (pcount)) return;
	unsigned int rid = sid + id;
	double3 upos = pos0[rid];
	upos.x += abs(0.04 * sin(2.0 * M_PI * (scte.time - stime)));
	double3 uvel = (upos - pos[rid]) / scte.dt;
	pos[rid] = upos;
	vel[rid] = uvel;
	auxVel[rid] = uvel;
}
