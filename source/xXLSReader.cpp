...
xPointMassData xXLSReader::ReadPointMassData(std::string& _name, int r, int& c, bool v)
{
	...
	xstring x;
	x = sheet->readStr(r, c++); x.split(",", 3, ptr + 1);
	x = sheet->readStr(r, c++); x.split(",", 3, ptr + 4);
	x = sheet->readStr(r, c++); x.split(",", 3, ptr + 7);
	x = sheet->readStr(r, c++); x.split(",", 4, ptr + 10);
	x = sheet->readStr(r, c++); x.split(",", 3, ptr + 14);
	if (xve && v)
	{
		int t = VMARKER;
...
xLineObjectData xXLSReader::ReadLineObjectData(std::string& _name, int mat, int r, int& c)
{
	...
	double* ptr = &d.p0x;
	xstring x;
	x = sheet->readStr(r, c++); x.split(",", 3, ptr + 0);
	x = sheet->readStr(r, c++); x.split(",", 3, ptr + 3);
	x = sheet->readStr(r, c++); x.split(",", 3, ptr + 6);
	if (xve)
	{
		int t = VLINE;
		xve->Write((char*)&t, sizeof(int));
...
xPlaneObjectData xXLSReader::ReadPlaneObjectData(std::string& _name, int mat, int r, int& c)
{
	...
	x = sheet->readStr(r, c++); x.split(",", 3, ptr + 3);
	x = sheet->readStr(r, c++); x.split(",", 3, ptr + 6);
	x = sheet->readStr(r, c++); x.split(",", 3, ptr + 9);
	if (xve)
	{
		int t = VPLANE;
		xve->Write((char*)&t, sizeof(int));
...
void xXLSReader::ReadJoint(xMultiBodyModel* xmbd, vector2i rc)
{
	while (1)
	{
		...
		if (!IsEmptyCell(rc.x, rc.y))
		{
			...
			xstring str = sheet->readStr(drc[0], drc[1]++);
			str.split(",", 2, drc);
			drc[0] -= 1; drc[1] -= 1;
			if (!IsEmptyCell(drc[0], drc[1]))
			{
				xDrivingConstraint* xdc = NULL;
				name = sheet->readStr(drc[0], drc[1]++);
				xdc = xmbd->CreateDrivingConstraint(name, xkc);
				double stime = sheet->readNum(drc[0], drc[1]++);
				double cvel = sheet->readNum(drc[0]++, drc[1]++);
	...
	if (xdem->XParticleManager())
	{
		...
		while (1)
		{
			...
			else if (form == CLUSTER_SHAPE)
			{
				...
				xstring x = sheet->readStr(rc.x, rc.y++);
				x.split(",", 3, &loc.x);
			...
			if (!IsEmptyCell(rc.x, rc.y))
			{
				...
				xstring x = sheet->readStr(rc.x, rc.y);
				if (x.size())// xUtilityFunctions::xsplit(sheet->readStr(rc.x, rc.y), ",", 2, &_rc.x))
				{
					x.split(",", 2, &_rc.x);
					...
					{
						np = static_cast<unsigned int>(sheet->readNum(_rc.x, _rc.y++));
						neach = static_cast<unsigned int>(sheet->readNum(_rc.x, _rc.y++));
						nstep = static_cast<unsigned int>(sheet->readNum(_rc.x, _rc.y));
					}
					/*		if (!neach)
								neach = npcircle;*/
					if (neach && nstep)
	...
	if (xom)
	{
		...
		while (1)
		{
			...
			else if (form == xShapeType::CLUSTER_SHAPE)
			{
				...
				xstring sdata;
				int loc[2] = { 0, };
				sdata = sheet->readStr(rc.x, rc.y++); sdata.split(",", 2, loc);
			...
			else if (form == xShapeType::MESH_SHAPE)
			{
				...
				vector3d loc;
				double fsz = 0.0;
				x = sheet->readStr(rc.x, rc.y++); x.split(",", 3, &(loc.x) + 0);
			...
			{
				double v[3] = { 0.0, };
				xstring ch_vel = sheet->readStr(rc.x, rc.y++);
				ch_vel.split(",", 3, v);
...
void xXLSReader::ReadInputGravity(vector2i rc)
{
	if (!IsEmptyCell(rc.x, rc.y))
	{
		xstring x;
		double v3[3] = { 0, };
		x = sheet->readStr(rc.x, rc.y); x.split(",", 3, v3 + 0);
