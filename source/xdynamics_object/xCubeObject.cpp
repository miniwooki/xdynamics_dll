...
bool xCubeObject::define(vector3d& min, vector3d& max)
{
	...
	planes[0].define(min_p, min_p + new_vector3d(0, 0, size.z), min_p + new_vector3d(size.x, 0, 0));
	planes[1].define(min_p, min_p + new_vector3d(0, size.y, 0), min_p + new_vector3d(0, 0, size.z));
	planes[2].define(min_p + new_vector3d(size.x, 0, 0), min_p + new_vector3d(size.x, 0, size.z), min_p + new_vector3d(size.x, size.y, 0));
	planes[3].define(min_p, min_p + new_vector3d(size.x, 0, 0), min_p + new_vector3d(0, size.y, 0));
	planes[4].define(min_p + new_vector3d(0, 0, size.z), min_p + new_vector3d(0, size.y, size.z), min_p + new_vector3d(size.x, 0, size.z));
	planes[5].define(min_p + new_vector3d(0, size.y, 0), min_p + new_vector3d(size.x, size.y, 0), min_p + new_vector3d(0, size.y, size.z));
	return true;
}
