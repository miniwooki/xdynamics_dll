...
xMassSpringTireModel::xMassSpringTireModel()
	: xMassSpringModel()
	, nattach(0)
	, aps(nullptr)
{
...
xMassSpringTireModel::xMassSpringTireModel(std::string _name)
	: xMassSpringModel(_name)
	, nattach(0)
	, aps(nullptr)
{
...
void xMassSpringTireModel::setTestModel()
{
	particles = new xParticleObject("mst_particles");
	//particles->AllocMemory()
}

void xMassSpringTireModel::allocAttachPoint(unsigned int n)
{
	aps = new xAttachPoint[n];
}
//
//void xMassSpringTireModel::allocRotationalSpring(unsigned int n)
//{
//	rsd = new xRotationalSpringData[n];
//}

void xMassSpringTireModel::appendPoint2Spring(xPoint2Spring * _p2s)
{
	//p2s.insert(_p2s->Name(), _p2s);
}

void xMassSpringTireModel::appendAttachPoint(unsigned int id, xAttachPoint & _xap)
{
	aps[id] = _xap;
}

void xMassSpringTireModel::appendRotationalSpring(xRotationSpringDamperForce* _xrd)
{
	rsda.insert(_xrd->Name(), _xrd);
}

void xMassSpringTireModel::calculateForce()
{
...
//xPoint2Spring * xMassSpringTireModel::Point2Spring(std::string n)
//{
//	return p2s.find(n).value();
//}

//void xMassSpringTireModel::setPointData(
