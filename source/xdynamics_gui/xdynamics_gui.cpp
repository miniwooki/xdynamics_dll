...
void xdynamics_gui::xGenerateClusterParticles(QString clusterName, QString name, int* dim, double* loc, bool israndom)
{
	...
	xParticleObject* pobj = pmgr->CreateClusterParticle(name.toStdString(), NO_MATERIAL, _loc, _dim, cobj, israndom);
	if (!xgl->vParticles())
		xgl->createParticles();
	if (xgl->vParticles())
	{
